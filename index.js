...
for (const folder of commandFolders) {
  const commandFiles = fs
    .readdirSync(`./util/${folder}`)
    .filter((file) => file.endsWith(".js")); //filter the command files
...
client.on("guildMemberAdd", (member) => {
  ...
  if (!channel) return;
  channel.send(
    `Welcome to the Stream Fam, ${member.toString()}! Don't forget to claim your welcome \`*hug\`! :purple_heart:`
  );
});
...
client.on("interactionCreate", (interaction) => {
  if (interaction.isButton()) {
    ...
    console.log(
      `No button/command detected in interaction event, exiting event...`
    );
    return;
  }
});
...
client.on("messageCreate", async function (message) {
  try {
    ...
    const urlRegexAlphanumeric = new RegExp(
      /(?:(?:(?:https|ftp|http|mailto|file|data|irc?):)?\/\/)?([\w\d\-]+\.)+\w{2,}(\/.+)?/gim
    );
    ...
    const urlRegexIPv6 = new RegExp(
      /(?:(?:(?:https|ftp|http|mailto|file|data|irc?):)?\/\/)?(([\da-fA-F]{4}:){1,7}[\da-fA-F]{4})/gim
    ); //Subst: /(?:(?:(?:https|ftp|http|mailto|file|data|irc?):)?\/\/)?(([\da-fA-F]{0,4}:){1,7}[\da-fA-F]{0,4})/
    ...
    if (!message.content.startsWith(prefix) || message.content.endsWith(prefix))
      return; //checks if the message starts or ends with *
      ...
      try {
        ...
        if (timestamps.has(message.author.id)) {
          const expirationDate =
            (await timestamps.get(message.author.id)) + cooldownAmount;
